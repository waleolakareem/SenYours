class AvailableDaysController < ApplicationController
  ...
  def show
    ...
    if(@availableDay.present?)
      @next_date =  AvailableDay.where({user_id:current_user}).where("date>?",@availableDay[0].date).order("date ASC")[0].try(:date)
      @prev_date =  AvailableDay.where({user_id:current_user}).where("date<?",@availableDay[0].date).order("date DESC")[0].try(:date)
    end
